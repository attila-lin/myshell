  ...
  void AddBC(const string* ops) //当前指令构构建成功，再加入一个
  {
    ...
    if(ops)
    {
      ...
      if(ops->compare("exit")==0)
        ...
        tmp=new ExecCmd;
      else if(ops->compare("alias")==0)
        tmp=new AliasCmd;
      else if(ops->compare("unalias")==0)
        tmp=new UnaliasCmd;
      else if(ops->compare("pwd")==0)
        tmp=new PwdCmd;
      else {
        // ExecManager* m = ExecManager::GetInstance();
        // // cout << m->GetAliasMap()->search(*ops);
        // if(m->GetAliasMap()->search(*ops).compare("exit")==0)
        //     tmp=new ExitCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("cd")==0)
        //     tmp=new CdCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("job")==0)
        //     tmp=new JobCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("kill")==0)
        //     tmp=new KillCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("echo")==0)
        //     tmp=new EchoCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("exec")==0)
        //     tmp=new ExecCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("alias")==0)
        //     tmp=new AliasCmd;
        // else if(m->GetAliasMap()->search(*ops).compare("pwd")==0)
        //     tmp=new PwdCmd;
        // else
        //    tmp=new ExternCmd;

        ExecManager* m = ExecManager::GetInstance();
        if(m->GetAliasMap()->search(*ops).compare("") != 0){
          // cout << m->GetAliasMap()->search(*ops) << endl;
          tmp = new ExecAliasCmd;
        }
        else
          tmp= new ExternCmd;
        // cout << m->GetAliasMap()->search(*ops) << endl;
    ...
    if(1==vpipel.size())
    {
        ...
        if(!vpipel[0]->IsExtern())
        {
            ...
            if(!vpipel[0]->RedirectIn())
              cerr<<"shell: Redirect In error"<<endl;
            if(!vpipel[0]->RedirectOut())
              cerr<<"shell: Redirect Out error"<<endl;

            // exit只有在单指令时才启作用
            if(vpipel[0]->Execute()==QUIT) return QUIT;
