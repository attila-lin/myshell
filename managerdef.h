...
public:
  ...
  void CheckBGTask(bool show=false)
  {
    pid_t back_pid;
    int count=0;
    for(list<pid_t>::iterator ilp=vbgtask.begin(); ilp!=vbgtask.end();)
    {
      int status;
...
ExecManager* ExecManager::m_Instance = NULL;
// AliasMap ExecManager::aliasmap = NULL;
#endif
